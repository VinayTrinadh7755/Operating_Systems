...
struct thread
  {
    ...
    struct list cl_ssv;             /* we are writing this list of child info structs*/
    struct thread *pt_ssv;          /* writing this parent thread pointer*/
    struct semaphore ws_ssv;        /* this detects wait semaphore and then signal parent to wait*/
    int esc_ssv;                /* this will set exit status via process exit*/
    bool extdc_ssv,wtdc_ssv;     /* will use this func to check if child is exited or not and the other func to check if wait function is already called or not*/               
#endif

    /* Owned by thread.c. */
    unsigned magic;                     /* Detects stack overflow. */
    struct file *rox_exec;
  };
